...
export default async function ViewStudentPage({ params }: EditStudentPageProps) {
  ...
  if (!response.success || !studentResult.success) {
    ...
    return (
      <div className="mx-8 h-full flex flex-col">
        <h1 className="font-bold text-lg mt-4">Visualizar Estudante</h1>
        <p className="text-sm text-gray-600 mt-2">
          Não foi possível carregar as informações para visualizar o estudante.
        </p>
  ...
  return (
    <Container className="mx-8 h-full flex flex-col">
      <div className="flex items-center justify-between p-1 bg-white mt-4">
        <h1 className="font-bold text-lg">Visualizar Estudante</h1>
      ...
      <Card className="mt-4 p-4">
        <EditStudentForm studentData={student} classes={classes!} readOnly/>
      </Card>
    </Container>
  );
}
